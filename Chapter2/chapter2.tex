...
\begin{itemize}
	...
	the project "Digital Solemn Declaration". The project is based on the use of APIs and its
	...
	actually being present in the agency's desks (KEP). It also aims to promote the digital convergence
	of Greece with other EU countries and to help Greece's digital transformation.
	This project as mentioned before is based on APIs. Specifically, it uses Django ~\citep{django} to create a
	REST API which designed according to the guidelines set by GOV.UK ~\citep{gov_uk_api} and shares data with other APIs vof public or private sector i.e Bank APIs in order to gather user information. \textit{"Django is a high-level Python Web framework that encourages rapid development and clean, pragmatic design. Built by experienced developers, it takes care of much of the hassle of Web development"}. Moreover, in terms of security OAuth2.0 Authorization Protocol ~\citep{oauth} is being used along with another identity protocol above OAuth2.0 called OpenId Connect~\citep{openidconnect}. \textit{"OpenID Connect 1.0 is a simple identity layer on top of the OAuth 2.0 protocol. It allows Clients to verify the identity of the End-User based on the authentication performed by an Authorization Server, as well as to obtain basic profile information about the End-User in an interoperable and REST-like manner. OpenID Connect allows clients of all types, including Web-based, mobile, and JavaScript clients, to request and receive information about authenticated sessions and end-users. The specification suite is extensible, allowing participants to use optional features such as encryption of identity data, discovery of OpenID Providers, and session management, when it makes sense for them."} The use of API in Digital Solemn Declaration permits open sharing of data through other Greek Government Agencies. This data can only be used by the Greek Government and by public sector agencies which means it is not accessible to third party developers.
